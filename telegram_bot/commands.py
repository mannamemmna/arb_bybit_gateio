...
class CommandHandler:
    ...
    async def cmd_auto(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == 'on':
            ...
            if self.engine:
                ...
                await update.message.reply_text(
                    f"🟢 *Engine STARTED*\n"
                    f"⚙️ Mode: `{self.engine.settings.trading_mode}`\n"
                    f"📊 Monitoring: `{len(self.engine.scanner.common_symbols)}` pairs",
                    parse_mode='MarkdownV2'
                )
        else:
            if self.engine:
                ...
                await update.message.reply_text(
                    f"🔴 *Engine STOPPED*\n"
                    f"📊 Session trades: `{self.engine.executor._execution_count}`",
                    parse_mode='MarkdownV2'
                )
    ...
    async def cmd_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        text += f"🔄 Mode: `{self.engine.settings.trading_mode}`\n"
        ...
        for exchange in ['bybit', 'gateio']:
            ...
            for conn in conns:
                ...
                text += f"{emoji} {exchange} \#{conn['index']}: `{conn['status']}` \({conn['symbols']} pairs\)\n"
    ...
    async def cmd_mode(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            f"{emoji} Mode switched to `{new_mode}`",
            parse_mode='MarkdownV2'
        )
    ...
    async def cmd_portfolio(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if positions:
            for pos in positions:
                text += f"• `{pos['symbol']}` {pos['direction']} \({pos['size_usdt']} USDT\)\n"
    ...
    async def cmd_history(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if args and args[0] == 'detail':
            ...
            for t in trades[:10]:
                ...
                emoji = '🟢' if net >= 0 else '🔴'
                text += f"{emoji} `{t['symbol']}` {t.get('direction','')} net=`{net:.4f}`\n"
        else:
            # Summary: 1D, 7D, 30D
            text = "📈 *Trade Summary*\n\n"
    ...
    async def cmd_top(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not spreads:
            ...
            for sym, spread in spreads[:5]:
                ...
                text += f"{direction} `{sym}`: `{spread:.3f}%`\n"
